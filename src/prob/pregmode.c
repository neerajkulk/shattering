...
void problem(DomainS *pDomain)
{
  ...
  for (j=js; j<=je; j++) {
    for (i=is; i<=ie; i++) {
      ...
      pGrid->U[0][j][i].M1 = dvx * rho; /* M1 is x momentum */
      pGrid->U[0][j][i].M2 = dvz * rho; /* M1 is y momentum */
    }
  }

  /* enroll special functions (Calling certian )*/
...
static void reflect_ix2(GridS *pGrid)
{
  ...
  iu = pGrid->ie + nghost;
  il = pGrid->is - nghost; /* Integrate over all cells including ghost cells */
  ...
  for (k=ks; k<=ke; k++) {
    for (j=1; j<=nghost; j++) {
      for (i=il; i<=iu; i++) {
        ...
        pGrid->U[k][js-j][i].E +=
          pGrid->U[k][js+(jc1)][i].d*1.0*(2*j-1)*pGrid->dx2/Gamma_1;
      }
    }
  }
