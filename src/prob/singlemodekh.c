...
//  init_cooling();
 ...
 /* divide by 2 so avg density of cool side is 1 w/ density perturbations by a factor of 2.5 ... then add a smooth interface.*/
  ...
  Real dbig=10.0;
  Real dsmal=0.1;
  Real n=2.0, amp=ly/50.0, thickness=ly/50.0;
  ...
  Real width = ly/4.0;
  Real intrfc;
  Real vflow = par_getd("problem","vflow");
    ...
    for (k=ks; k<=ke; k++) {
        for (j=js; j<=je; j++) {
            for (i=is; i<=ie; i++) {
                ...
                intrfc = tanh(a*(width+x2))*(dsmal-dbig)/2.0 + (dsmal+dbig)/2.0;
               
		vx=tanh(a*(width+x2))*(vflow)/2.0 + vflow/2.0;
                
                vy=-0.02*vflow*sin(2.0*PI*x1/lx)*exp(-(x2+width)*(x2+width)/(thickness*thickness));
		...
		pGrid->U[k][j][i].M2 = pGrid->U[k][j][i].d*vy;
