    ...
    for (k=ks; k<=ke; k++) {
        for (j=js; j<=je; j++) {
            for (i=is; i<=ie; i++) {
                ...
                Real dbig=1.0;
                Real dsmal=0.1;
                Real n=5.0, amp=0.1, thickness=lx/10.0;
                Real a = 2.0*atanh(0.9)/thickness;
                Real vb = par_getd("problem","boost");
		Real b = par_getd("problem","bloby");
                Real vx=0.0;
                Real r,theta,rintrfc,smoother;

		Real noisearray[ie-is][je-js][ks-ke];
		...
		rintrfc = 0.25*sqrt(SQR(lx)+SQR(ly))*(1+amp*cos(n*theta));

                
                smoother = tanh(a*(rintrfc-r))*(dbig-dsmal)/2.0 + (dbig+dsmal)/2.0;

		noisearray[i][j][k] = pGrid->U[k][j][i].d;

		
		pGrid->U[k][j][i].d = 0.1*exp(noisearray[i][j][k]/2.0)*(1.0-b) +b*smoother;
