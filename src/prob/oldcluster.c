...
/*=== READ ME ===*/
/*This is the (mostly) pristine version of the code Mike originally gave me*/
/*What is not pristine is some incorrect code to try to calculate the radial profiles*/
/*This is not the most up to date version of the code but I'm a hoarder and fear I may */
/*need this some day so I've simply renamed it as "oldcluster"*/
/*Whats the take away? I should've started using git earlier... stupid*/
/*This change is just to test the whole git thing*/
...
static void calc_profiles(DomainS *pDomain, Real **profile_data)
{
   ...
   for(k=ks; k<=ke; k++){
      for(j=js; j<=je; j++){
	   ...
	   r = sqrt(SQR(1.0*(i-is)-0.5*Nx) + SQR(1.0*(j-js)-0.5*Ny) + SQR(1.0*(k-ks)-0.5*Nz));
            /*FIX ME: Make binning more accurate*/
            s = (int) floor(r);
	    if (s >= sqrt(3.0)*Nx/2){
	      ath_error("problem: Nbins=%d, Nx=%d, i,j,k=%d,%d,%d, s=%d\n",
			n_bins, Nx, i, j, k, s);
	    }
...
/*     /\* next, calculate the radial profiles and store them in the global array *\/ */
/*     calc_profiles(&(pM->Domain[nl][nd]), */
/* 		  radius, */
/* 		  d_profile, */
/* 		  press_profile, */
/* 		  temp_profile, */
/* 		  mach_profile); /\* Here you dont pass nmax *\/ */
