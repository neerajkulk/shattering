...
static void perturb_vel(GridS *pGrid);
...
static void perturb(GridS *pGrid)
{
  ...
  for (k=ks; k<=ke; k++) {
    for (j=js; j<=je; j++) {
      for (i=is; i<=ie; i++) {
        pGrid->U[k][j][i].d *= exp(2.0 + dd[k][j][i]);

        if (pGrid->U[k][j][i].d > 10.0)
          pGrid->U[k][j][i].d = 10.0 + sqrt(pGrid->U[k][j][i].d - 10.0);
      }
    }
  }
...
void problem(DomainS *pDomain)
{
  ...
  int k, ks=pGrid->ks, ke = pGrid->ke;
  int ixs,jxs,kxs;
  Real x1, x2, x3, r, theta;
  Real b = par_getd("problem", "blob_frac");
  Real lx,ly;
  ...
  for (k=ks; k<=ke; k++) {
	...
	Real n = 5.0;
	Real dnew, rintrfc;
	Real thickness=lx/10.0;
	Real a = 2.0*atanh(0.9)/thickness;

	pGrid->U[k][j][i].d = pGrid->U[k][j][i].d/(10.67) + 0.07;

       	rintrfc = lx*0.25*(1+amp*cos(n*theta));	

        dnew =  tanh(a*(rintrfc-r))*(dbig-dsmal)/2.0 + (dbig+dsmal)/2.0;

        pGrid->U[k][j][i].d = b * dnew + (1.0-b) * pGrid->U[k][j][i].d;
      }
    }
  }
